...
export default function SideBar(props) {
    ...
    return(
        <div className= {props.sideBar? `${styles.sidebar} ${styles.open}`: `${styles.sidebar} ${styles.closed}`}>
          ...
          <Link href="/movies"  onClick={() =>props.setSideBar(false)}>Movies</Link>
          ...
          <Link href="/fixtures"  onClick={() =>props.setSideBar(false)}>Fixtures</Link>
          <Link href="/tables"  onClick={() =>props.setSideBar(false)}>Tables</Link>
        </div>
    )
}