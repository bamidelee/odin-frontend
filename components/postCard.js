...
export default function PostCard({ news, title, page, type, pageCount, pageLink, country }) {
        ...
        <div className={styles.postCard}>
           ...
           {news && <DashCard dashPosts={news} title= {`${title.charAt(0).toUpperCase()}${title.slice(1)}`} page={pageLink} mini= {true} country = {country}/>}
            ...
            {count > 8 && <div className={styles.pagnation}>
                ...
                <Link href={title === 'Latest movies' ? `/page/${title}/${count}/latestMovies` : `/page/${title.toLowerCase()}/${count}/${type}`} className={parseInt(page) === count ? styles.activePage : styles.inactivePage}>{count}</Link>
            </div>}
        </div>
    )
}