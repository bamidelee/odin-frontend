import { NEWS_PAGE, LATEST_MOVIES, PAGE_COUNT, MOVIE_COUNT, LATEST_SERIES, LATEST_SERIES_COUNT, FIND_CONTENT_BY_COUNTRY, COUNTRY_COUNT, REQUEST_MOVIES, REQUEST_MOVIE_COUNT, REQUEST_SERIES, REQUEST_SERIES_COUNT } from "../../components/quarries"
...
export default function News({ news, latestMovies, pageCount, latestMoviesCount, latestSeries, latestSeriesCount, country, countryCount, requestMovies, requestSeries, requestMoviesCount, requestSeriesCount }) {
  ...
  return (
    <div>
      ...
      <PostCard news={param[2] === 'latestMovies' ? latestMovies : param[2] === 'requestMovies' ? requestMovies : param[2] === 'latestSeries' ? latestSeries : param[2] === 'requestSeries' ? requestSeries : param[3] === 'country' ? country : news} title={param[0]} page={param[1]} type={param[2]} pageCount={param[2] == 'latestMovies' ? latestMoviesCount : param[2] == 'requestMovies' ? requestMoviesCount : param[2] === 'latestSeries' ? latestSeriesCount : param[2] === 'requestSeries' ? requestSeriesCount : param[3] ==='country' ? countryCount : pageCount} pageLink={(param[2] === 'latestMovies' || param[2] === 'movie' || param[2] === 'requestMovies') ? 'movies' : 'series'} countryName={param[3]} />
...
export async function getStaticProps({ params }) {
  ...
  const { data: requestMovieData } = await client.query({ query: REQUEST_MOVIES, variables: { pageNumber: param[1] } });
  const { data: latestSeriesData } = await client.query({ query: LATEST_SERIES, variables: { pageNumber: param[1] } });
  const { data: requestSeriesData } = await client.query({ query: REQUEST_SERIES, variables: { pageNumber: param[1] } });
  ...
  const { data: requestMovieCountData } = await client.query({ query: REQUEST_MOVIE_COUNT });
  const { data: seriesCountData } = await client.query({ query: LATEST_SERIES_COUNT });
  const { data: requestSeriesCountData } = await client.query({ query: REQUEST_SERIES_COUNT });
  ...
  return {
    props: {
      ...
      requestMovies: requestMovieData ? requestMovieData.requestMovies : null,
      pageCount: pageCountData.pageCount.count,
      latestMoviesCount: movieCountData.latestMoviesCount.count,
      requestMoviesCount: requestMovieCountData.requestMoviesCount.count,
      latestSeries: latestSeriesData.latestSeries,
      requestSeries: requestSeriesData.requestSeries,
      latestSeriesCount: seriesCountData.latestSeriesCount.count,
      requestSeriesCount: requestSeriesCountData.requestSeriesCount.count,
